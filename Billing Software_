...
class Bill_App:
    def __init__(self, root): 
        ...
        self.root.title("Billing Software") # Titile
        bg_color = "#badc57"
        title = Label(self.root, text="Billing Software", font=('times new roman', 35, 'bold'), 
                    pady=2, bd=12, bg="#7F7FFF", fg="BLACK", relief=GROOVE)
        title.pack(fill=X)
    # ================variables=======================
        self.sanitizer = IntVar() # IntVar() is used to store the value of the entry widget
        self.mask = IntVar()
    ...
    # ==============Total product price================
        self.medical_price = StringVar() #StringVar is a class in Tkinter that represents a variable that holds a string value. 
    ...
    # ==============Customer==========================
        ...
        x = random.randint(1000, 9999) #  generating the bill number as random 
    ...
    # ===================Medical====================================
        ...
        bandage_lbl = Label(F2, text="Bandage", font =('times new roman', 16, 'bold'), bg = "#badc57", fg = "black")
        bandage_lbl.grid(row=4, column=0, padx=10, pady=10, sticky='W')
        bandage_txt = Entry(F2, width=10, textvariable=self.bandage, font=('times new roman', 16, 'bold'), bd=5, relief=GROOVE)
        bandage_txt.grid(row=4, column=1, padx=10, pady=10)
    ...
    # ===========ColdDrinks================================
        ...
        limca_lbl = Label(F4, text="Limca", font=('times new roman', 16, 'bold'), bg="#badc57", fg="black")
...
#=========billArea=================================================
    def bill_area(self):
        if self.c_name.get() == " " or self.c_phone.get() == " ":
            ...
            self.welcome_bill()
    # Validation part what will happen when other are
    # ============medical===========================
        ...
        if self.bandage.get() != 0:
            self.txtarea.insert(END, f"\n bandage\t\t{self.bandage.get()}\t\t{self.m_n_p}")
    ...
    # ==============Grocery============================
        ...
        if self.limca.get() != 0:
            self.txtarea.insert(END, f"\n Sanitizer\t\t{self.limca.get()}\t\t{self.c_d_l_p}")
        if self.mazza.get() != 0:
        ...
        if self.mountain_dew.get() != 0:
            self.txtarea.insert(END, f"\n Mountain Dew\t\t{self.mountain_dew.get()}\t\t{self.c_m_d}")
            self.txtarea.insert(END, f"\n--------------------------------")
        # ===============taxes==============================
